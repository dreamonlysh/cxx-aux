...
class simple_any {
  ...
  using __Buffer =
      std::aligned_storage_t<sizeof(void*), std::alignment_of<void*>::value>;
  ...
  // The handler is the polymorphism with different operation for the small
  // talk.
...
ValueType* simple_any_cast(simple_any&) {
  // [optional] The __Handler with different address(template with type) for
  // different type. [optional] use type info [optional] template <typename
  // ValueType> struct unique_id {static auto value = 0;}; the address of value
  // is unique type id.
  return nullptr;
}
