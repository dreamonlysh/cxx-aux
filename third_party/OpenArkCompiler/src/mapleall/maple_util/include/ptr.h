...
public:
  ...
  constexpr Ptr() noexcept : ptr_(nullptr) { Check(nullptr); }

  constexpr explicit Ptr(std::nullptr_t) noexcept : ptr_(nullptr) {
    Check(nullptr);
  }

  explicit Ptr(pointer ptr) : ptr_(ptr) { Check(ptr); }

  Ptr(pointer ref, PtrCheckerType<T> checker) : ptr_(ref) { checker(ptr_); }

  explicit Ptr(T& ref) : ptr_(&ref) { Check(ptr_); }

  Ptr(T& ref, PtrCheckerType<T> checker) : ptr_(&ref) { checker(ptr_); }
  ...
  template <typename U,
            typename = std::enable_if_t<std::is_convertible<U, T>::value>>
  explicit Ptr(U& ref) : ptr_(&ref) {
    Check(ptr_);
  }

  template <typename U,
            typename = std::enable_if_t<std::is_convertible<U, T>::value>>
  Ptr(U& ref, PtrCheckerType<T> checker) : ptr_(&ref) {
    checker(ptr_);
  }
  ...
  Ptr(const Ptr& other) : ptr_(other.get()) {}

  Ptr(Ptr&& other) noexcept : ptr_(other.get()) {}

  template <typename U, PtrCheckerType<U> CheckU,
            typename = std::enable_if_t<std::is_convertible<U, T>::value>>
  explicit Ptr(const Ptr<U, CheckU>& other) : ptr_(other.get()) {
    Check(ptr_);
  }
  ...
  template <typename U, PtrCheckerType<U> CheckU,
            typename = std::enable_if_t<std::is_convertible<U, T>::value>>
  explicit Ptr(Ptr<U, CheckU>&& other) : ptr_(other.get()) {
    Check(ptr_);
  }

  template <typename U, PtrCheckerType<U> CheckU,
            typename = std::enable_if_t<std::is_convertible<U, T>::value>>
  Ptr(Ptr<U, CheckU>&& other, PtrCheckerType<T> checker) : ptr_(other.get()) {
    checker(ptr_);
  }
  ...
  void swap(Ptr& other) noexcept { std::swap(ptr_, other.ptr_); }
...
template <typename T, PtrCheckerType<T> CheckT, typename U,
          PtrCheckerType<U> CheckU>
