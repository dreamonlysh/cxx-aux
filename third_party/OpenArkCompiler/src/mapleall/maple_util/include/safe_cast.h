...
template <typename ToT, typename FromT>
struct instance_of_impl<
    ToT, FromT, typename std::enable_if_t<std::is_base_of<ToT, FromT>::value>> {
  static inline bool DoIt(const FromT&) { return true; }
};

template <typename ToT, typename FromT>
struct enabled_safe_cast : utils::meta_or<std::is_base_of<ToT, FromT>,
                                          safe_cast_condition<ToT>>::type {};
} // namespace __impl

template <
    typename ToT, typename FromT,
...
template <
    ...
    typename RetT = std::conditional_t<
        std::is_const<FromT>::value ||
            std::is_const<std::remove_pointer_t<ToT>>::value,
