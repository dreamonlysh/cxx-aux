...
public:
  ...
  std::unique_ptr<TObject> Create(const key_type& key, TArgs... args) const {
    ...
    return it == creator.end() ? std::unique_ptr<TObject>()
...
template <typename TFactory, typename TFactory::key_type Key,
          typename TObjectImpl>
...
inline auto CreateProductObject(const typename TFactory::key_type& key,
                                TArgs&&... args) {
...
inline void RegisterFactoryFunction(const typename TFactory::key_type& key,
                                    typename TFactory::creator_type func) {
