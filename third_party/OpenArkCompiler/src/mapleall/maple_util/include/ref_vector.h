...
// The `estd::ref_vector<T>` is a decorator of `std::vector<T*>` which won't
// destory any `T`s and just refer to them.
//
// It is designed mainly to solve the problem that only the owner who is very
// familiar can tell whether the `T*` can be nullptr. As time goes by, the
// others, even the owner may forget it. Or with more coding base on it, some
// will start to check nullptr, it will make the code more confusion and hard to
// maintain. Based on that, the ref_vector picks up the meaning of keeping
// objects' reference, it won't destory any of its elements or require to check
// null(Never to be nullptr).
//
// etc.
// `std::vector<T*>`
// ```
...
struct mpl_iterator_traits<Iterator, ref_vector_tag> {
  ...
  static constexpr Iterator operator_arrow(Iterator iter) { return *iter; }
...
public:
  ...
  using const_iterator =
      mpl_iterator<typename base_vector::const_pointer, ref_vector_tag>;
  ...
  // There shouldn't be any situation that initialize this with all the same
  // reference. Maybe an initial small size.
  explicit ref_vector(size_type count, const value_type& value,
                      const allocator_type& alloc = allocator_type()) = delete;
  explicit ref_vector(size_type count,
                      const allocator_type& alloc = allocator_type()) = delete;

  // The pointer is stored in the base, and the Iter owns objects instead of
  // their pointers.
  template <typename Iter>
  ref_vector(Iter first, Iter last,
             const allocator_type& alloc = allocator_type()) = delete;
  ...
  ref_vector(std::initializer_list<value_type> init,
             const allocator_type& alloc = allocator_type()) = delete;
  ...
  reverse_iterator rbegin() noexcept { return reverse_iterator(base.rbegin()); }

  const_reverse_iterator rbegin() const noexcept {
    return const_reverse_iterator(base.rbegin());
  }

  reverse_iterator rend() noexcept { return reverse_iterator(base.rend()); }
  ...
  reference at(size_type pos) { return *(base.at(pos)); }
  ...
  void push_back(value_type& value) { base.push_back(&value); }
  ...
  void pop_back() noexcept { base.pop_back(); }
  ...
  iterator insert(const_iterator pos,
                  std::initializer_list<value_type> init) = delete;

  template <typename... Args>
  iterator emplace(const_iterator pos, Args&&... args) = delete;

  iterator erase(const_iterator pos) { return iterator(base.erase(pos)); }

  iterator erase(const_iterator first, const_iterator last) {
    return iterator(first.base(), last.base());
  }

  void swap(ref_vector& other) noexcept { base.swap(other.base); }
...
inline bool operator==(const ref_vector<T, Alloc>& lhs,
                       const ref_vector<T, Alloc>& rhs) {
...
namespace std {
template <typename T, typename Alloc>
inline void swap(maple::utils::ref_vector<T, Alloc>& lhs,
                 maple::utils::ref_vector<T, Alloc>& rhs) noexcept {
  lhs.swap(rhs);
}
} // namespace std
#endif
