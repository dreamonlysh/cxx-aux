...
public:
  OpCluster() {
    initialize_once<factory_type>({{'+', &OpCluster::plus},
                                   {'-', &OpCluster::minus},
                                   {'*', &OpCluster::star},
                                   {'/', &OpCluster::div}});
  }
  int plus(int lhs, int rhs) { return (lhs + rhs) * scale; }

  int minus(int lhs, int rhs) { return (lhs - rhs) * scale; }

  int star(int lhs, int rhs) { return (lhs * rhs) * scale; }

  int div(int lhs, int rhs) { return (lhs / rhs) * scale; }
...
TEST(OpCluster, Div) { ASSERT_EQ(OpCluster().calc('/', 1, 2).value_or(-1), 0); }
...
public:
  VoidOpCluster() {
    initialize_once<factory_type>({{'+', &VoidOpCluster::plus},
                                   {'-', &VoidOpCluster::minus},
                                   {'*', &VoidOpCluster::star},
                                   {'/', &VoidOpCluster::div}});
  }
  void plus(int lhs, int rhs) { rst = (lhs + rhs) * scale; }

  void minus(int lhs, int rhs) { rst = (lhs - rhs) * scale; }

  void star(int lhs, int rhs) { rst = (lhs * rhs) * scale; }

  void div(int lhs, int rhs) { rst = (lhs / rhs) * scale; }

  void calc(char c, int lhs, int rhs) { invoke<factory_type>(c, lhs, rhs); }

  int result() const { return rst; }
